...
class ONNXBackend(BaseModelBackend):
    ...
    def load_model(self, w):
        ...
        if self.device.type == "mps":
            # self.checker.check_packages(("onnxruntime-silicon==1.20.0",))  # DISABLED: SSL issues
            providers = ["MPSExecutionProvider", "CPUExecutionProvider"]
        elif self.device.type == "cuda":
            # self.checker.check_packages(("onnxruntime-gpu==1.20.0",))  # DISABLED: SSL issues
            providers = ["CUDAExecutionProvider", "CPUExecutionProvider"]
        else:
            # self.checker.check_packages(("onnxruntime==1.20.0",))  # DISABLED: SSL issues
