...
class BaseModelBackend:
    ...
    def get_features(self, xyxys, img):
        ...
        end_time = time.time()
        reid_time = end_time - start_time
        if xyxys.size != 0:
            num_objects = len(xyxys)
            print(f"🔍 ReID Feature Extraction: {num_objects} objects, Time: {reid_time * 1000:.2f} ms, Avg per object: {reid_time * 1000 / num_objects:.2f} ms")
        return features
