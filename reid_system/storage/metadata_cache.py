...
class TrackMetadataCache:
    ...
    def should_upsert_person_vector(
        ...
        Simplified for demo: Accept ALL vectors for both PENDING and ASSIGNED tracks.
        """
        lock = self._get_track_lock(track.local_id)
        with lock:
            # Always update EMA and accept
            self._ema_update(track, new_person_vector, alpha=self.EMA_ALPHA)
            return True

    def mark_person_extract(self, track: TrackMetadata, frame_num: int, vector: np.ndarray, crop: Optional[np.ndarray] = None) -> None:
        lock = self._get_track_lock(track.local_id)
        with lock:
            ...
            # Store crop for later commit (will be saved when global_id is assigned)
            if crop is not None:
            ...
            if not track.global_id:
                ...
                # Store crop in history (RAM-based during PENDING)
                if crop is not None:
                    track.crop_history.append((
                        crop.copy(),
                        frame_num,
                    ))
                ...
                # Also trim crop_history to match
                while len(track.crop_history) > self.MAX_HISTORY_VECTORS:
                    track.crop_history.pop(0)
    ...
    def commit_person_vectors(
        ...
        # Use provided crop or track's last_crop
        crop_to_save = crop_image if crop_image is not None else track.last_crop
        ...
        ok_snap = self.vector_store.upsert_snapshot(
            ...
            crop_image=crop_to_save,  # Pass crop for ASSIGNED tracks
        )
        
        # Clear crop after commit to free memory
        if ok_snap:
            track.last_crop = None
    ...
    def batch_commit_person_vectors(
        ...
        for track, vector in tracks_to_commit:
            ...
            # Add crop if track has one (for visual debugging)
            has_crop = hasattr(track, 'last_crop') and track.last_crop is not None
            if has_crop:
                entry["crop_image"] = track.last_crop
                logger.info(f"📸 BATCH_COMMIT_CROP: track={track.local_id} gid={track.global_id} crop_shape={track.last_crop.shape}")
            else:
                logger.info(f"⚠️ BATCH_COMMIT_NO_CROP: track={track.local_id} gid={track.global_id} has_attr={hasattr(track, 'last_crop')} is_none={getattr(track, 'last_crop', None) is None}")
        ...
        # Single batch upsert call with crops
        committed = self.vector_store.batch_upsert_with_crops(
            entries=entries,
            stream_id=stream_id,
        )
        
        # Clear crops after commit to free memory
        for track, _ in tracks_to_commit:
            if hasattr(track, 'last_crop'):
                track.last_crop = None
        
        return committed
    
    def commit_vector_and_crop_history(
        self,
        track: TrackMetadata,
        stream_id: str,
    ) -> Tuple[int, int]:
        """
        Commit both vectors and crops from history when track gets assigned.
        
        IMPORTANT: Must commit vectors FIRST to get matrix indices,
        then commit crops with those same indices.
        
        Args:
            track: Track with history to commit
            stream_id: Stream ID for folder structure
            
        Returns:
            Tuple of (vectors_committed, crops_written)
        """
        if not track.global_id:
            return (0, 0)
        
        logger.info(f"🔄 COMMIT_VECTOR_AND_CROP_HISTORY: track={track.local_id} gid={track.global_id} vector_history={len(track.person_vector_history)} crop_history={len(track.crop_history)}")
        
        # NOTE: Vectors are already committed in _finalize_assign_new, so we don't commit them again
        # Just commit crops with the existing matrix indices
        
        # Step 2: Commit crops with the same matrix indices
        # Note: crop_history and person_vector_history must be synchronized
        crops_written = 0
        if track.crop_history and self.vector_store:
            # Get the matrix indices that were just assigned
            track_key = f"{stream_id}:{track.local_id}"
            if track_key in self.vector_store.track_to_indices:
                indices = list(self.vector_store.track_to_indices[track_key])
                # Match crops with their corresponding matrix indices
                num_crops = min(len(track.crop_history), len(indices))
                for i in range(num_crops):
                    crop_img, frame_num = track.crop_history[i]
                    matrix_idx = indices[i]  # Use actual matrix index for FIFO consistency
                    if crop_img is not None:
                        # IMPORTANT: Use matrix_idx (not i) for FIFO compatibility
                        # When FIFO evicts vector at matrix_idx, it deletes corresponding crop
                        self.vector_store._save_crop_image(
                            crop=crop_img,
                            stream_id=stream_id,
                            track_id=track.local_id,
                            global_id=track.global_id,
                            matrix_index=matrix_idx,  # Use actual matrix index
                        )
                        crops_written += 1
            
        
        # Clear history after writing (only if crops were actually written)
        if crops_written > 0:
            track.person_vector_history.clear()
            track.crop_history.clear()
        
        return 0, crops_written
    ...
    def commit_person_vector_history(
        ...
        committed = self.vector_store.batch_upsert_snapshots(
            ...
            crop_image=track.last_crop if hasattr(track, 'last_crop') else None,
        )

        # Only clear history if vectors were actually committed
        if committed > 0:
            track.person_vector_history.clear()
