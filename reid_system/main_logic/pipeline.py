...
class ReIDPipeline:
    ...
    def process_reid_frame(
        ...
        self.logger.debug("Objects type: %s, content: %s", type(objects), objects)
        ...
        # Create person crops from frame and bboxes
        person_list = []
        for obj in person_objects:
            ...
            if len(bbox) >= 4 and frame is not None:
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    person_data = {
                        'id': obj.get('track_id', obj.get('id')),
                        'confidence': obj.get('confidence', 0.0),
                        'capture': crop,  # This is the crop ReID needs
                        'bbox': bbox
                    }
        ...
        try:
            ...
            self.logger.debug("Global ID mappings extracted: %s", global_id_mappings)
